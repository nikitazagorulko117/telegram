...
#игра друзей:
import random
N = int(input("введите сколько участников:"))
action = input("(напишите задание для проигравшего).Кто-то... ")
safe = []
for i in range(N):
    name = input("введите имя: ")
    number = int(input(f"Введите число от 1 до {N}, которое выберет {name}: "))
    safe.append((name, number))
loss = random.randint(1,N)
for name, number in safe:
    if number == loss:
        print(f'{name:} {action}')
print(f"(число проигравшего: {loss})")
'''
#Бот-города
import telebot
